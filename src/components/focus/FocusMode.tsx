      ...
      >
        <div className="mx-auto flex min-h-[calc(100dvh-3rem)] max-w-4xl flex-col">
          <header className="flex items-center justify-between gap-4">
            ...
            <Button variant="ghost" size="sm" onClick={exit} aria-label="Exit Focus Mode">
            ...
            <AnimatePresence mode="wait">
              {completion ? (
                <motion.div key="complete" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-xl">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-accent">Focus complete</p>
                  <h1 className="mt-4 text-4xl font-semibold tracking-tight sm:text-6xl">Great work.</h1>
                  <p className="mt-4 text-muted">{completionTask?.title ?? 'Your session'} · +{completion.durationMinutes} min focused</p>
                  <div className="mt-8 flex flex-wrap justify-center gap-3">
                    <Button variant="accent" size="lg" onClick={() => { clearCompletion(); start() }}><Play size={16} /> Start Break</Button>
                    <Button variant="secondary" size="lg" onClick={finishCompletion}>Finish</Button>
                  </div>
                </motion.div>
              ) : (
                <motion.div key="timer" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-2xl">
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-muted">{labels[mode]}</p>
                  <p className="mt-5 min-h-7 text-lg font-medium text-text">{task?.title ?? 'Choose a task to begin'}</p>
                  ...
                  <div className="mx-auto mt-10 h-2 w-full max-w-xl overflow-hidden rounded-full bg-elevated" aria-label={`${Math.round(progress * 100)}% complete`}>
                  ...
                  <div className="mt-10 flex flex-wrap justify-center gap-3">
                    ...
                    <Button variant="secondary" size="lg" onClick={endSession}>End Session</Button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <footer className="flex flex-col items-center justify-between gap-4 border-t border-border pt-5 text-xs text-muted sm:flex-row">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="sm" onClick={() => updateSettings({ soundEnabled: !soundEnabled })} aria-label={soundEnabled ? 'Mute completion sound' : 'Enable completion sound'}>
            ...
            <span>Space pause/resume · F focus mode · Esc exit</span>
          </footer>
          ...
          <AnimatePresence>
            {showReminder && !reminderDismissed ? (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="fixed bottom-5 left-1/2 z-[60] flex w-[calc(100%-2rem)] max-w-md -translate-x-1/2 items-center gap-3 rounded-2xl border border-border bg-card/95 px-4 py-3 text-sm shadow-2xl backdrop-blur-xl">
                ...
                <span className="flex-1">For deeper focus, consider enabling Do Not Disturb on your device.</span>
                <button type="button" onClick={() => setReminderDismissed(true)} className="shrink-0 text-muted hover:text-text" aria-label="Dismiss Do Not Disturb reminder">Dismiss</button>
              </motion.div>
            ) : null}
          </AnimatePresence>
        </div>
